...
# retail places
business <- read.socrata("https://data.sfgov.org/resource/g8m3-pdis.json")
retail_spot <- business  %>% 
  filter(is.na(location_end_date)) %>% 
  filter(grepl("4400-4599",naic_code)) %>%
#  dplyr::select(ttxid, naic_code) %>% 
  distinct()
...
parking_net <- 
  ...
  mutate(countPark = replace_na(countPark, 0),
         uniqueID = rownames(.),
         cvID = sample(round(nrow(meter_net) / 8), 
...
### ?Joining off-street parking to the fishnet

```{r Joining off-street parking to the fishnet}
# Off-street Parking
...
### ?Combining all factors into one fishnet
