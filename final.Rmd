...
ggplot() +
  geom_sf(data = graffiti_net, aes(fill = countGraffit), color = NA) +
  scale_fill_viridis() +
...
street_parks.final <- street_parks.final[sample(nrow(street_parks),100000),]  
...
street_park_grid <-
  st_join(var_net,street_parks) %>%
  na.omit() 

park.panel <- 
  expand.grid(interval60=unique(street_park_grid$interval30), 
              uniqueID = unique(street_park_grid$uniqueID)) %>%
  left_join(., street_park_grid %>%distinct())
