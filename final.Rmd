...
park.panel <- 
  expand.grid(interval15=unique(street_park_grid$interval15), 
...
park.panel.count <- 
  ...
  summarize(Real_Count = mean(Park_Counter, na.rm=T)) %>%
  ungroup() %>%
...
park.panel.lag <- 
  ...
  mutate(lag15min =  dplyr::lag(Real_Count,0.25),        #cannot use dplyr::lag() to create a 15-minute lag
         lagHour = dplyr::lag(Real_Count,1),
         lag2Hours = dplyr::lag(Real_Count,2),
         lag3Hours = dplyr::lag(Real_Count,3)) %>%
  mutate(day = yday(interval15))

ggplot(park.panel.lag, aes(interval15, Real_Count)) + 
  geom_line() +
