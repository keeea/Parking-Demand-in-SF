...
We select six weeks, which is 07/16/2021 - 08/26/2021 (week29 - week34), as the time window to train and test models. First, load on-street parking data of meters and records. As it is plotted below, most of the on-street parking behaviors cluster in the northeastern section of San Francisco, including Financial District, China Town, Fisherman's Wharf, etc, where the daytime parking occupancy is closer to 80% and traffic congestion is more serious. So, we trimmed our study area into these seven neighborhoods: North Beach, Russian Hill, Nob Hill, Chinatown, Financial District/South Beach, Tenderloin, South of Market.
...
ggplot() + 
  geom_sf(data = neighborhood_sf,fill="transparent",size=0.1,) +
  geom_sf(data = study_area, colour=palette2[2], size=1,fill="transparent") +
  labs(title= "Study Areas for Parking in San Francisco")+
  mapTheme()
```
...
For all point-level data, like meters, parking records, police incidents, and etc., the way to aggregate data into fishnet is to get the count of point by cell.
