...
Because most on-street parking in San Francisco will be free during 18:00 - 9:00, and meters won't be working and charging during that time, therefore we only have rare data for that time range. What's more, predicting that non-peak time when there are few cars parking is not significant for our use case. So we have trimmed data into opening hours.
...
```{r Creating models, echo=TRUE, message=TRUE, warning=TRUE, paged.print=TRUE, results=}
OLS_reg1 <- 
...
## Selecting kitchen sink models

Given all AIC, MEA and predicted/observed time series, `OLS_reg2` and `OLS_reg3` performs best. Moreover, `OLS_reg3` is more flexible and generalizable, on account of that the spatial effects in the model is the equilibrium of different factors of environment, road and neighborhood, rather than a fixed number (which cannot be directly applied on surrounding cities).

Next, we will further optimize `OLS_reg3` with more feature engineering.

## Engineered models

### Further manual engineering

First, time features, like hour, are more suitable to be modeled as categorical feature, so transform them into character. Then plot the correlation matrix for numeric variables, and remove variables that are significantly correlated with others. So graffiti and mean commute time are deleted to reduce the multicollinearity of the model.
...
### Stepwise to optimize

At last, put new variables into Stepwise regression to screen out the best model.
...
week_predictions.3 <- 
  park.Test.weekNest.2 %>% 
    mutate(F_OLS_reg4 = map(.x = data, fit = OLS_reg4, .f = model_pred))
...
After Stepwise regression, the algorithm remove predictors of parking violation and street speed limit. As the result of all additional feature engineering above, the R-squared improves from 0.7779 to 0.7845, and the second MAE (MAE for normalized parking value) decreases from 0.68 to 0.60.
...
Here we generate an animated map of the MAEs by grid and by hour to see the spatio-temporal distribution of the model's MAE. We can see that for most of the time periods and most of the spatial units, our model perform well with a satisfying MAE.
...
To better test the generalizability of the OLS regression model, we apply cross validation here. The distribution of MAE cluster quite tightly together (most of which are from 3.3 to 3.8 approximately), and this suggest that the model have a quite good generalizability.
...
reg.cv <- 
  train(Real_Count ~ hour + dotw + lagHour + lag2Hours + lag3Hours + 
    ...
    data = st_drop_geometry(park.Train), 
